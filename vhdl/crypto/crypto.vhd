...
architecture rtl of crypto is
    ...
    -- Decoupled AXI Write Latches
    signal aw_pending  : std_ulogic := '0';
    signal w_pending   : std_ulogic := '0';
    signal aw_addr_lat : std_ulogic_vector(11 downto 0) := (others => '0');
    signal w_data_lat  : std_ulogic_vector(31 downto 0) := (others => '0');
    ...
    -- Decoupled AXI Read Latches
    signal ar_pending    : std_ulogic := '0';
    signal ar_addr_lat_r : std_ulogic_vector(11 downto 0) := (others => '0');

    -- DMA master output intermediates (VHDL-93 cannot read 'out' ports)
    signal m0_arvalid_int : std_ulogic;
    signal m0_awvalid_int : std_ulogic;
    signal m0_rready_int  : std_ulogic;
    signal m0_wvalid_int  : std_ulogic;
...
begin
    ...
    -- Drive m0_axi outputs from intermediates
    m0_axi_arvalid <= m0_arvalid_int;
    m0_axi_awvalid <= m0_awvalid_int;
    m0_axi_rready  <= m0_rready_int;
    m0_axi_wvalid  <= m0_wvalid_int;
    ...
    port map(
        ...
        arvalid => m0_arvalid_int,
        arready => m0_axi_arready,
        rdata => m0_axi_rdata,
        rresp => m0_axi_rresp,
        rvalid => m0_axi_rvalid,
        rready => m0_rready_int,
    ...
    port map(
        ...
        awvalid => m0_awvalid_int,
        awready => m0_axi_awready,
        wdata => m0_axi_wdata,
        wstrb => m0_axi_wstrb,
        wvalid => m0_wvalid_int,
    ...
    -- AXI Write Process (Decoupled Handshake)
    process(aclk)
    begin
        if rising_edge(aclk) then
            if aresetn = '0' then
                ...
                -- Latch Address
                if s0_axi_awvalid = '1' and s0_axi_awready_int = '0' and aw_pending = '0' then
                    s0_axi_awready_int <= '1';
                    aw_pending <= '1';
                    aw_addr_lat <= s0_axi_awaddr;
                else
                    s0_axi_awready_int <= '0';
                end if;

                -- Latch Data
                if s0_axi_wvalid = '1' and s0_axi_wready_int = '0' and w_pending = '0' then
                    s0_axi_wready_int  <= '1';
                    w_pending <= '1';
                    w_data_lat <= s0_axi_wdata;
                else
                    s0_axi_wready_int  <= '0';
                end if;

                -- Process Write when both are available
                if aw_pending = '1' and w_pending = '1' and s0_axi_bvalid_int = '0' then
                    aw_pending <= '0';
                    w_pending  <= '0';
                    s0_axi_bvalid_int <= '1';
                    s0_axi_bresp_int  <= "00";

                    case aw_addr_lat(11 downto 2) is
                        when "0000000000" => -- 0x00: Bit 0 = Start
                            reg_start <= w_data_lat(0);
                        when "0000000001" => -- 0x04: AAD Base Addr
                            reg_aad_base_addr <= w_data_lat;
                        when "0000000010" => -- 0x08: AAD Length
                            reg_aad_byte_length <= w_data_lat;
                        when "0000000011" => -- 0x0C: Plaintext Base Addr
                            reg_payload_base_addr <= w_data_lat;
                        when "0000000100" => -- 0x10: Plaintext Length
                            reg_payload_byte_length <= w_data_lat;
                        when "0000000101" => -- 0x14: Ciphertext Base Addr
                            reg_cipher_base_addr <= w_data_lat;
                        when "0000000110" => -- 0x18: Ciphertext Length
                            reg_cipher_byte_length <= w_data_lat;
                        when "0000000111" => -- 0x1C: Key Length Config
                            reg_key_len <= w_data_lat(1 downto 0);
                        when "0000001000" => reg_key(255 downto 224) <= w_data_lat; -- 0x20
                        when "0000001001" => reg_key(223 downto 192) <= w_data_lat; -- 0x24
                        when "0000001010" => reg_key(191 downto 160) <= w_data_lat; -- 0x28
                        when "0000001011" => reg_key(159 downto 128) <= w_data_lat; -- 0x2C
                        when "0000001100" => reg_iv(95 downto 64)    <= w_data_lat; -- 0x30
                        when "0000001101" => reg_iv(63 downto 32)    <= w_data_lat; -- 0x34
                        when "0000001110" => reg_iv(31 downto 0)     <= w_data_lat; -- 0x38
                        when "0000010100" => reg_key(127 downto 96)  <= w_data_lat; -- 0x50
                        when "0000010101" => reg_key(95 downto 64)   <= w_data_lat; -- 0x54
                        when "0000010110" => reg_key(63 downto 32)   <= w_data_lat; -- 0x58
                        when "0000010111" => reg_key(31 downto 0)    <= w_data_lat; -- 0x5C
                        when others =>
                            s0_axi_bresp_int <= "11";
                    end case;
                end if;
    ...
    -- AXI4 Read (decoupled: accept address, then present data)
    process(aclk)
    begin
        if rising_edge(aclk) then
            if aresetn = '0' then
                ...
                ar_pending         <= '0';
                ar_addr_lat_r      <= (others => '0');
            else
                -- Phase 1: Accept address whenever we don't have one pending
                --          and we are not still holding rvalid from a previous read
                if s0_axi_arvalid = '1' and ar_pending = '0' and s0_axi_rvalid_int = '0' then
                    s0_axi_arready_int <= '1';
                    ar_pending         <= '1';
                    ar_addr_lat_r      <= s0_axi_araddr;
                else
                    s0_axi_arready_int <= '0';
                end if;

                -- Phase 2: Present data one cycle after accepting address
                if ar_pending = '1' and s0_axi_arready_int = '0' then
                    ar_pending        <= '0';
                    s0_axi_rvalid_int <= '1';
                    s0_axi_rresp_int  <= "00";

                    case ar_addr_lat_r(11 downto 2) is
                        when "0000000000" =>
                        ...
                        when "0000000111" =>
                            s0_axi_rdata_int <= (1 => reg_key_len(1), 0 => reg_key_len(0), others => '0');
                        -- KEY
                        when "0000001000" => s0_axi_rdata_int <= reg_key(255 downto 224);
                        when "0000001001" => s0_axi_rdata_int <= reg_key(223 downto 192);
                        when "0000001010" => s0_axi_rdata_int <= reg_key(191 downto 160);
                        when "0000001011" => s0_axi_rdata_int <= reg_key(159 downto 128);
                        -- IV
                        when "0000001100" => s0_axi_rdata_int <= reg_iv(95 downto 64);
                        when "0000001101" => s0_axi_rdata_int <= reg_iv(63 downto 32);
                        when "0000001110" => s0_axi_rdata_int <= reg_iv(31 downto 0);
                        -- DEBUG register at 0x3C
                        when "0000001111" =>
                            s0_axi_rdata_int <= (
                                0 => dma_in_idle,
                                1 => dma_out_idle,
